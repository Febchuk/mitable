    ...
    <div className="p-4 md:p-6 pb-24 md:pb-6 max-w-3xl space-y-4">
      ...
      <div className="border border-stroke-subtle rounded-xl overflow-hidden bg-canvas-raised">
        {classStudents.map((s, i) => {
          // draft is synced from initialDraft via useEffect, but on the
          // render where classStudents updates, the effect hasn't run yet.
          // Fall back to initialDraft so the row still renders.
          const entry = draft[s.id] ?? initialDraft[s.id];
      ...
      <div className="flex items-center justify-between">
        ...
        <Button variant="accent" onClick={save} disabled={!dirty || saveAttendance.isPending}>

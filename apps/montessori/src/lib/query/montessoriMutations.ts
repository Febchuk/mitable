...
import type { GridSnapshot } from "@/lib/query/montessoriQueries";
import type { ProposedUpdatesEnvelope } from "@/types/proposed-updates";
import type { InputMethod, MasteryLevel, Observation, Report, ReportStatus } from "@/types";
...
export function useSetObservation() {
  ...
  return useMutation({
    mutationFn: async (input: SetObservationInput) => {
      // classroomId is for cache invalidation only — the API doesn't take it.
      const body = {
    ...
    // Optimistic write: drop a synthesized observation into the cached
    // grid so the cell re-colors before the network round-trip lands.
    // The grid uses the FIRST matching obs for (student, topic) as the
    // current level, so we prepend.
    onMutate: async (vars) => {
      const key = montessoriKeys.grid(vars.classroomId);
      await qc.cancelQueries({ queryKey: key });
      const previous = qc.getQueryData<GridSnapshot>(key);
      if (previous) {
        const optimistic: Observation = {
          ...
          inputMethod: vars.inputMethod ?? "grid",
          authorType: vars.authorType ?? "teacher",
          authorId: "optimistic",
        };
        qc.setQueryData<GridSnapshot>(key, {
          ...previous,
          observations: [optimistic, ...previous.observations],
        });
      }
      return { previous };
    },
    onError: (_err, vars, context) => {
      if (context?.previous) {
        qc.setQueryData(montessoriKeys.grid(vars.classroomId), context.previous);
      }
    },
...
export function useInterpretCapture() {
  return useMutation({
    mutationFn: async (input: InterpretCaptureInput): Promise<InterpretCaptureResult> => {
      ...
      return apiRequestForm<InterpretCaptureResult>("/montessori/agent/interpret", form);
    },
  });
}
...
export function useUpdateReport() {
  ...
  return useMutation({
    mutationFn: async (input: UpdateReportInput) => {
      // classroomId is for cache invalidation only — the API doesn't take it.
      const { id, classroomId, ...body } = input;
      void classroomId;

...
const ParameterRow = ({ name, desc, importance, isLast, C }: { name: string; desc: string; importance: number; isLast: boolean; C: ReturnType<typeof getMockupColors> }) => (
    <div
        style={{
            display: "flex",
            alignItems: "center",
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                    ...
                    <div style={{ display: "inline-flex", gap: 3, background: `rgba(${C.uiRgb}, 0.05)`, borderRadius: 7, padding: 3 }}>
                        {["Weekly", "Monthly", "Quarterly"].map((f) => (
                            <div key={f} style={{ padding: "4px 10px", borderRadius: 5, fontSize: 11, color: f === "Monthly" ? C.text : C.textTer, background: f === "Monthly" ? (isLight ? "rgba(28,43,51,0.08)" : "rgba(255,255,255,0.08)") : "transparent" }}>
                                {f}
                            </div>
                        ))}
                    </div>

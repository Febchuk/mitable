...
const CUSTOMERS = [
    { label: "Acme Corp", pct: 38, hours: 14.2, colorKey: "accent" as const },
    { label: "Initech", pct: 26, hours: 9.8, colorKey: "accentDark" as const },
    { label: "Globex", pct: 21, hours: 7.9, staticColor: true },
    { label: "Stark Ind.", pct: 15, hours: 5.6, staticColor: true },
];
...
function getCustomerColor(index: number, C: ReturnType<typeof getMockupColors>) {
    const colors = [C.accent, C.accentDark, "#A5C4A0", "#7E8AA2"];
    return colors[index] ?? C.textTer;
}

const DonutChart = ({ C }: { C: ReturnType<typeof getMockupColors> }) => {
    const size = 120;
    const sw = 12;
    const r = (size - sw) / 2;
    ...
    let offset = 0;
    const arcs = CUSTOMERS.map((c, i) => {
        const dash = (c.pct / 100) * circ;
        const arc = { dash, gap: circ - dash, offset, color: getCustomerColor(i, C) };
        offset += dash;
        return arc;
    });
...
const BarRow = ({ label, pct, minutes, C }: { label: string; pct: number; minutes: number; C: ReturnType<typeof getMockupColors> }) => (
...
export const PersonDetailMockup = ({ variant = "dark" }: { variant?: MockupVariant }) => {
        ...
        >
            ...
            <div className="l-pd-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, flex: 1, minHeight: 0, overflow: "hidden" }}>
                <div
                    style={{
                        background: C.raised,
                        border: `1px solid rgba(${C.uiRgb}, 0.04)`,
                        borderRadius: 12,
                        padding: "16px 18px",
                        display: "flex",
                        flexDirection: "column",
                        ...
                        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8, overflow: "hidden" }}>
                            {CUSTOMERS.map((c, i) => (
                                <div key={c.label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                    <div style={{ width: 14, height: 3, borderRadius: 999, background: getCustomerColor(i, C), flexShrink: 0 }} />
                ...
                >
                    <span style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.09em", color: C.textSec, marginBottom: 14 }}>
                        Activity Breakdown
                    </span>

...
const SlackScene = ({ charCount, C }: { charCount: number; C: MockupColors }) => {
    ...
    const messages = [
        { user: "SK", name: "Sarah Kim", time: "2:14 PM", text: "Any updates on the SSO integration for Acme?" },
        {
            ...
            name: "Tunde Bakare",
            time: "2:18 PM",
            text: "Just finished configuring their IdP — tested all auth flows and they\u2019re passing \u2713",
        },
                ...
                >
                    ...
                    {showTips && (
                        <div style={{ fontSize: 11, color: C.textTer, marginTop: 8 }}>
                            <span style={{ color: C.accent }}>/help</span> for help &nbsp;\u00B7&nbsp; <span style={{ color: C.accent }}>/status</span> for
                            setup
                        </div>
                    )}
                </div>

...
const SocialIcon = ({ href, label, children }: { href: string; label: string; children: React.ReactNode }) => (
    <a
        href={href}
        aria-label={label}
        className="flex size-9 items-center justify-center rounded-lg text-gray-600 transition-all duration-200 hover:bg-gray-800/60 hover:text-gray-300"
    >
        {children}
    </a>
);
                        ...
                        <div key={category}>
                            ...
                            <ul className="flex flex-col gap-2.5">
                                {footer.links[category].map((link) => (
                                    <li key={link.label}>
                                        <a href={link.href} className="text-sm text-gray-500 transition-colors duration-200 hover:text-white">
                                            {link.label}
                                        </a>
                                    </li>
                                ))}

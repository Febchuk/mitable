...
import { ChevronDown, ChevronRight, Activity, Terminal, Code, Globe, FileText, Image as ImageIcon } from "lucide-react";
...
function getActivityIcon(appName: string | null, activity: string | null) {
  ...
  if (text.includes("code") || text.includes("cursor") || text.includes("vs")) return <Code className="w-3.5 h-3.5" />;
  if (text.includes("terminal") || text.includes("warp") || text.includes("iterm")) return <Terminal className="w-3.5 h-3.5" />;
  if (text.includes("chrome") || text.includes("browser") || text.includes("safari")) return <Globe className="w-3.5 h-3.5" />;
    ...
    <div ref={entryRef} className={`relative ${!isLast ? "pb-2" : ""}`}>
      ...
      >
        ...
        <div className="relative flex-shrink-0 mt-1">
          <div className={`w-5 h-5 rounded-full border border-border-default bg-background-elevated flex items-center justify-center text-text-tertiary ${isPending ? 'animate-pulse' : ''}`}>
        ...
        <div className="flex-1 min-w-0 pt-0.5">
          <div className="flex items-center gap-2">
            <span className={`text-sm ${isPending ? 'text-text-tertiary italic' : 'text-text-secondary'}`}>

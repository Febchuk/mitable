...
export default function SessionStats({ stats, className = "" }: SessionStatsProps) {
  return (
    <div
      className={`flex flex-wrap items-center gap-x-8 gap-y-3 py-3 border-b border-stroke-subtle ${className}`}
    >
      {/* Total Time */}
      <StatItem label="Total Time" value={formatDuration(stats.totalTimeMinutes)} />
      ...
      <StatItem
        ...
        subValue={
          stats.interruptionMinutes > 0 ? formatDuration(stats.interruptionMinutes) : undefined
        }
      />
...
function StatItem({ label, value, subValue, accent }: StatItemProps) {
  return (
    <div className="flex items-baseline gap-2">
      ...
      <span
        className={`text-sm font-medium tabular-nums ${accent ? "text-indigo" : "text-ink-primary"}`}
      >
        {value}
      </span>
      {subValue && <span className="text-xs text-ink-tertiary">({subValue})</span>}
    </div>
  );
}

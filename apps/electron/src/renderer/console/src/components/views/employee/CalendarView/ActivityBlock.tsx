...
export default function ActivityBlock({
  ...
  // Render markdown summary for Granola meetings
  const renderedSummaryHtml = useMemo(() => {
    if (!isGranola || !block.summary) return "";
    const result = marked.parse(block.summary);
    return DOMPurify.sanitize(typeof result === "string" ? result : "");
  }, [isGranola, block.summary]);

  // Accent colors differ for Granola meetings vs work blocks
  const accentColor = isGranola ? GRANOLA_GREEN : "#9B84E8";
  ...
  return (
    <div
      style={{
        background: isGranola && isExpanded ? "rgba(200, 230, 74, 0.02)" : "#211F1B",
    ...
    >
      ...
      {isExpanded && (
        <div
          style={{
            borderTop: `0.5px solid ${isGranola ? "rgba(200, 230, 74, 0.08)" : "rgba(236, 232, 224, 0.06)"}`,
            padding: "0 16px 16px",
          }}
        >
          {/* ── Granola meeting body ── */}
          {isGranola && (
            <>
              ...
              {block.subscriberName && (
                ...
                >
                  <User size={11} style={{ color: GRANOLA_GREEN, flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: GRANOLA_GREEN }}>{block.subscriberName}</span>
                </div>
              )}

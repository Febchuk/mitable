  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="font-medium text-ink-primary truncate">{workstream.name}</span>
      </div>
      ...
      {/* Row 3: Apps used */}
      {appsDisplay && (
        <div className="text-xs text-ink-tertiary truncate mb-1" title={appsDisplay}>
          {appsDisplay}
        </div>
      )}

      {/* Row 4: Time ranges */}
      <div className="text-xs text-ink-tertiary truncate tabular-nums" title={timeRanges}>
        {timeRanges}
      </div>
    </div>
  );

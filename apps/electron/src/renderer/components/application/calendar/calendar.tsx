...
import {
  ...
  type EventViewColor,
} from "./base-components/calendar-month-view-event";
...
const MonthView = ({
  ...
  const days = useMemo(
    () => getDaysInVisibleMonthGrid(currentMonthDate, locale),
  ...
  const weekdays = useMemo(() => {
    ...
    return Array.from({ length: 7 }).map((_, i) =>
      shortWeekdayFormatter.format(firstDay.add({ days: i }).toDate(timeZone))
    );
  ...
  const targetDateForFooter =
    selectedDate || (isSameMonth(currentMonthDate, todayCalDate) ? todayCalDate : null);
  ...
  const eventRenderMap = useMemo(() => {
    ...
    const isWeekBoundaryStart = (
      dayIndex: number,
      eventStartDay: CalendarDate,
    ...
    <div className={cx("flex flex-1 flex-col", className)}>
      ...
      >
        {days.map((date, index) => {
          ...
          const isLastRow =
            days.length > 35 ? index >= 35 : days.length > 28 ? index >= 28 : index >= 21;
            ...
            >
              <div className="flex gap-1 max-md:pl-1 md:flex-col">
                {eventsToShow.map(({ event, span, isPlaceholder }) => {
                  ...
                  const eventStartsAtDayStart = isSameDay(
                    eventStartDay,
                    toCalendarDate(getStartOfDay(event.start, timeZone))
                  );
                  const eventEndsAtDayEnd = isSameDay(
                    eventEndDay,
                    toCalendarDate(getEndOfDay(event.end, timeZone))
                  );
                  ...
                  const supportingText = isAllDayForCell
                    ? undefined
                    : formatTimeForMonth(event.start);
      ...
      {targetDateForFooter && (
        <div className="border-t border-stroke-subtle px-4 py-5 md:hidden">
          <h3 className="text-sm font-semibold text-ink-primary">
            {fullDateFormatter.format(targetDateForFooter.toDate(timeZone))}
          </h3>
          ...
          {eventsForFooter.length > 0 && (
            <div className="mt-4 flex flex-col gap-1.5">
              {eventsForFooter.slice(0, 3).map((event) => {
                ...
                const isAllDay =
                  event.start.compare(eventStartDay) <= 0 && event.end.compare(eventEndDay) >= 0;
          ...
          {eventsForFooter.length === 0 && (
            <p className="mt-4 text-xs font-semibold text-quaternary">No events for this day.</p>
          )}
          {eventsForFooter.length > 3 && (
            <p className="mt-4 text-xs font-semibold text-quaternary">{`${eventsForFooter.length - 3} more...`}</p>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className={cx("relative flex-1", className)}>
      ...
      {dayEvents.map((event, index) => {
        const overlapIndex = dayEvents.filter(
          (e, i) => i < index && event.start.compare(e.end) < 0 && event.end.compare(e.start) > 0
        ).length;
        const totalOverlapping = dayEvents.filter(
          (e) => event.start.compare(e.end) < 0 && event.end.compare(e.start) > 0
        ).length;
...
const WeekViewDay = ({
  ...
  const dayEvents = useMemo(
    () => getEventsForDay(visibleEvents, day, timeZone),
  ...
  return (
    <div className="flex flex-col border-stroke-subtle">
      <div className="relative flex-1 bg-canvas-base" style={{ minHeight: `${48 * slotHeight}px` }}>
        {Array.from({ length: 48 }).map((_, slotIndex) => (
          <CalendarDwViewCell
            key={`slot-${dateKey}-${slotIndex}`}
            className={cx("last:before:border-b-0", isLastDay && "before:border-r-0")}
          />
        ))}
...
const WeekView = ({
  ...
  const showTimeMarker =
    localCurrentTime.compare(toZoned(currentWeekStart, timeZone)) >= 0 &&
  ...
  if (showTimeMarker) {
    ...
    const minutesFromTop =
      localCurrentTime.hour * 60 +
      localCurrentTime.minute -
      (startOfVisibleDay.hour * 60 + startOfVisibleDay.minute);
    ...
    <div className={cx("flex flex-1 flex-col overflow-auto bg-canvas-base", className)}>
      <div className="sticky top-0 z-10 grid grid-cols-7 bg-canvas-raised pl-18 border-b border-stroke-subtle">
        {days.map((day) => {
          ...
          const dayToHighlight =
            selectedDate && day.compare(currentWeekStart) >= 0 && day.compare(currentWeekEnd) <= 0
              ? selectedDate
              : currentWeekStart;
      ...
      <div ref={scrollContainerRef} className="relative flex flex-1 overflow-y-auto">
        ...
        <div className="flex h-max w-18 flex-col border-r border-stroke-subtle">
          {hours.map((hour) => {
            const time = new CalendarDateTime(
...
const DayView = ({
  ...
  const visibleEvents = useMemo(
    () => getEventsForDay(zonedEvents, dayToDisplay, timeZone),
    [zonedEvents, dayToDisplay, timeZone]
  );
          ...
          >
            ...
            <AriaCalendarGrid weekdayStyle="short" className="w-max">
              <AriaCalendarGridHeader className="border-b-4 border-transparent">
                {(day) => (
                  <AriaCalendarHeaderCell className="p-0">
                    <div className="flex size-10 items-center justify-center text-sm font-medium text-ink-secondary">
                      {day.slice(0, 2)}
                    </div>
              ...
              <AriaCalendarGridBody className="[&_tr]:last-of-type]:border-none [&_td]:p-0 [&_tr]:border-b-4 [&_tr]:border-transparent">
                {(date) => (
                  <CalendarCell date={date} isHighlighted={highlightedDates.has(date.toString())} />
                )}
...
export const Calendar = ({
  ...
  const controlledDate = currentDate
  ...
  const headerDate = useMemo(
    () =>
      view === "day" && selectedDate
        ...
        : currentMonthDate.toDate(timeZone),

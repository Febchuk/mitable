...
export const CalendarMonthViewEvent = ({
  label,
  supportingText,
  color = "gray",
  collapseOnMobile = false,
}: CalendarMonthViewEventProps) => {
  return (
    <>
      <div
        className={cx(
          "inline-flex size-2 items-center justify-center md:hidden",
      ...
      >
        <div className="flex w-full items-center justify-between gap-0.5">
          <span
            className={cx("flex-1 truncate text-xs font-semibold", eventViewColors[color].label)}
          >
            {label}
          </span>
          {supportingText && (

...
export function WorkflowAccordion({
  ...
  return (
    <div className="workflow-accordion my-4 border border-white/10 rounded-2xl overflow-hidden bg-white/5">
      {/* Accordion Header - only show when workflow is active */}
      {!isPreFlight && (
        <button
          className="accordion-header w-full px-4 py-3 flex items-center justify-between hover:bg-white/5 transition-colors"
        ...
        >
          <div className="flex items-center gap-3">
            ...
            {workflow.status === "completed" ? (
              ...
              <Circle size={18} className="text-yellow-500 flex-shrink-0" />
            ) : (
              <Circle size={18} className="text-[#8B5CF6] flex-shrink-0" />
            )}

            {/* Title & Progress */}
            <div className="text-left">
              <h3 className="text-sm font-semibold text-white">{workflow.solution}</h3>
      ...
      {(isPreFlight || isExpanded) && (
        <div className={cn("accordion-body", !isPreFlight && "border-t border-white/10")}>
          ...
          <div className="workflow-steps px-4 pb-3">
            {workflow.stepList.map((step: any, idx: number) => {
              ...
              return (
                <div
                  key={idx}
                  className={cn(
                    "workflow-step-section rounded-lg mb-2 transition-all",
                    isCurrentStep && "bg-white/10 border-2 border-[#8B5CF6]",
                    !isCurrentStep && "bg-white/5 border border-white/10",
                ...
                >
                  ...
                  <button
                    className={cn(
                      ...
                      isExpandable && "hover:bg-white/5 cursor-pointer",
                  ...
                  >
                    ...
                    {!isPreFlight && hasMessages && (
                      <span className="message-count text-xs text-gray-400 bg-black/30 px-2 py-0.5 rounded-full flex-shrink-0">
                        {stepMessages.length} msg{stepMessages.length !== 1 ? "s" : ""}
                      </span>
                    )}
                  ...
                  {!isPreFlight && (isCurrentStep || expandedSteps.has(idx)) && (
                    <div className="step-content border-t border-white/10 px-3 pb-3 pt-2">
                      {/* Plan adjustment notice if applicable */}
                      {isCurrentStep && workflow.adjustmentHistory.length > 0 && (
                        <div className="plan-adjustment-notice bg-yellow-500/20 border border-yellow-500/40 rounded p-2 mb-2">
                      ...
                      {stepMessages.map((msg, msgIdx) => (
                        <div
                          key={msgIdx}
                          className={cn(
                            "step-message mb-2 p-2 rounded-lg",
                            msg.role === "user"
          ...
          {(isPreFlight || workflow.status === "paused" || workflow.status === "completed") && (
            <div className="px-4 py-3 border-t border-white/10">
              {isPreFlight ? (
                ...
                <button
                  className="resume-workflow-btn w-full flex items-center justify-center gap-2 bg-[#8B5CF6] text-white px-4 py-2 rounded-full hover:bg-[#8B5CF6]/90 transition-all hover:scale-105 font-medium text-sm"

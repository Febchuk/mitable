// electron.vite.config.ts
import { defineConfig, externalizeDepsPlugin } from "electron-vite";
import react from "@vitejs/plugin-react";
import { resolve } from "path";
var __electron_vite_injected_dirname = "/Users/febechukwuma/Documents/mitable/apps/electron";
var getVitePort = () => {
  if (process.env.VITE_PORT) {
    const parsed = parseInt(process.env.VITE_PORT, 10);
    if (!isNaN(parsed) && parsed >= 1 && parsed <= 65535) {
      return parsed;
    }
  }
  return 5173;
};
var VITE_PORT = getVitePort();
var excludeMainProcessPlugin = () => {
  const rendererLoggerPath = resolve(__electron_vite_injected_dirname, "src/renderer/lib/logger.ts");
  const mainLoggerPath = resolve(__electron_vite_injected_dirname, "src/lib/logger.ts");
  return {
    name: "exclude-main-process",
    resolveId(id) {
      if (id === mainLoggerPath || id.includes("/src/lib/logger") && !id.includes("/src/renderer/")) {
        return rendererLoggerPath;
      }
      return null;
    },
    load(id) {
      if (id === mainLoggerPath || id.includes("/src/lib/logger.ts") && !id.includes("/src/renderer/")) {
        const normalizedPath = rendererLoggerPath.replace(/\\/g, "/");
        return `export * from "${normalizedPath}";`;
      }
      return null;
    }
  };
};
var electron_vite_config_default = defineConfig({
  main: {
    plugins: [externalizeDepsPlugin({ exclude: ["electron-store"] })],
    build: {
      rollupOptions: {
        input: {
          index: resolve(__electron_vite_injected_dirname, "src/main.ts")
        },
        output: {
          format: "cjs",
          // CommonJS has native __dirname (ESM import.meta.dirname needs Node 20.11+)
          entryFileNames: "[name].cjs"
          // .cjs extension ensures CommonJS despite "type": "module" in package.json
        },
        external: [
          // WebSocket optional dependencies (native modules, not needed in Electron)
          "bufferutil",
          "utf-8-validate"
        ]
      }
    }
  },
  preload: {
    plugins: [externalizeDepsPlugin({ exclude: ["electron-log"] })],
    build: {
      rollupOptions: {
        input: {
          console: resolve(__electron_vite_injected_dirname, "src/preload/console.ts"),
          watchButton: resolve(__electron_vite_injected_dirname, "src/preload/watchButton.ts"),
          watchingPill: resolve(__electron_vite_injected_dirname, "src/preload/watchingPill.ts"),
          watchingPillDropdown: resolve(__electron_vite_injected_dirname, "src/preload/watchingPillDropdown.ts"),
          notification: resolve(__electron_vite_injected_dirname, "src/preload/notification.ts")
        },
        output: {
          format: "cjs",
          entryFileNames: "[name].cjs"
        }
      }
    }
  },
  renderer: {
    root: resolve(__electron_vite_injected_dirname, "src/renderer"),
    server: {
      port: VITE_PORT,
      strictPort: true,
      // Fail if port is already in use (no auto-increment)
      fs: {
        // Deny access to main process and preload files
        deny: [
          resolve(__electron_vite_injected_dirname, "src/main.ts"),
          resolve(__electron_vite_injected_dirname, "src/lib"),
          resolve(__electron_vite_injected_dirname, "src/preload"),
          resolve(__electron_vite_injected_dirname, "src/main"),
          resolve(__electron_vite_injected_dirname, "src/services")
        ]
      }
    },
    resolve: {
      alias: {
        "@": resolve(__electron_vite_injected_dirname, "src/renderer"),
        // Redirect main process logger imports to renderer logger
        // This prevents Vite from trying to process src/lib/logger.ts (main process file)
        [resolve(__electron_vite_injected_dirname, "src/lib/logger.ts")]: resolve(__electron_vite_injected_dirname, "src/renderer/lib/logger.ts"),
        // Stub out electron-log modules to prevent Vite from trying to bundle them
        "electron-log": resolve(__electron_vite_injected_dirname, "src/renderer/lib/electron-log-stub.ts"),
        "electron-log/renderer": resolve(__electron_vite_injected_dirname, "src/renderer/lib/electron-log-stub.ts"),
        "electron-log/main": resolve(__electron_vite_injected_dirname, "src/renderer/lib/electron-log-stub.ts")
      }
    },
    optimizeDeps: {
      // Exclude electron-log from pre-bundling - it uses Node.js globals and doesn't work in renderer
      exclude: ["electron-log", "electron-log/renderer", "electron-log/main"]
    },
    plugins: [react(), excludeMainProcessPlugin()],
    // Exclude main process files from being processed by Vite
    publicDir: false,
    build: {
      rollupOptions: {
        input: {
          console: resolve(__electron_vite_injected_dirname, "src/renderer/console/index.html"),
          watchButton: resolve(__electron_vite_injected_dirname, "src/renderer/watchButton/index.html"),
          watchingPill: resolve(__electron_vite_injected_dirname, "src/renderer/watchingPill/index.html"),
          watchingPillDropdownEye: resolve(__electron_vite_injected_dirname, "src/renderer/watchingPillDropdown/eye.html"),
          watchingPillDropdownMenu: resolve(
            __electron_vite_injected_dirname,
            "src/renderer/watchingPillDropdown/menu.html"
          ),
          notifications: resolve(__electron_vite_injected_dirname, "src/renderer/notifications/index.html")
        }
      }
    }
  }
});
export {
  electron_vite_config_default as default
};

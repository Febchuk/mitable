 ...
 * 4. Type-based boosting - SQL schemas, code blocks, tables get priority
...
export class NotionRetriever {
   ...
   * 3. Merge with RRF (60% semantic, 40% keyword - balanced for technical docs)
   * 4. Type-based boosting (8x for SQL, 5x for code, 3x for tables)
   * 5. Light recency boost (optional, less than Slack)
   * 6. Group by page for coherent context
   */
  async retrieve(
    query: string,
    ...
    // Step 3: Type-based boosting (use structure-aware metadata!)
    const typeBoosted = merged.map((block) => {
      // Use chunk_type if available (structure-aware), fallback to block_type (old)
    ...
    console.log(
      `[NotionRetriever] Found ${pages.length} pages (${topBlocks.length} blocks) in ${searchTime}ms`
    );
  ...
  private async semanticSearch(
    ...
    console.log(
      `[NotionRetriever] Semantic search: ${results.length} blocks (filtered at query time)`
    );
  ...
  private async keywordSearch(
    ...
    return results.map((r) => ({
      ...
      // Structure-aware metadata
      sectionPath: r.sectionPath ? JSON.parse(r.sectionPath) : undefined,
      sectionTitle: r.sectionTitle || undefined,
      sectionId: r.sectionId || undefined,
      headingLevel: r.headingLevel || undefined,
      chunkType: r.chunkType || undefined,
      hasCode: r.hasCode || false,
      hasTable: r.hasTable || false,
      hasList: r.hasList || false,
   ...
   */
  private mergeWithRRF(semantic: NotionBlock[], keyword: NotionBlock[]): NotionBlock[] {
  ...
  private groupByPage(blocks: NotionBlock[]): NotionPage[] {
    ...
    return Array.from(pagesMap.values()).sort((a, b) => b.topScore - a.topScore);
  }

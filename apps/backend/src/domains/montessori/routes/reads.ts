...
router.get("/classrooms/:id", async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const [row] = await db
            ...
            .from(montessoriClassrooms)
            .where(
                and(eq(montessoriClassrooms.id, id), eq(montessoriClassrooms.organizationId, orgId))
            )
            .limit(1);
        if (!row) {
...
router.get("/students/:id", async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const [row] = await db
            ...
            .from(montessoriStudents)
            .where(
                and(eq(montessoriStudents.id, id), eq(montessoriStudents.organizationId, orgId))
            )
            .limit(1);
        if (!row) {
...
router.get("/students/:id/attendance", async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const rows = await db
            ...
            .where(
                and(
                    eq(montessoriAttendance.studentId, id),
                    eq(montessoriAttendance.organizationId, orgId)
                )
            )
        ...
        const [students, entries] = await Promise.all([
            ...
            db
                ...
                .where(
                    and(
                        eq(montessoriAttendance.organizationId, orgId),
                        eq(montessoriAttendance.date, date)
                    )
                ),
        ]);

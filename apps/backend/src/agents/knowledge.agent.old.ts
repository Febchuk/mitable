  ...
  async *execute(context: ToolContext): AsyncIterable<StreamChunk> {
    try {
      ...
      const tools: Groq.Chat.ChatCompletionTool[] = [
        {
          ...
          function: {
            ...
            description: "Search the company knowledge base (Slack conversations and Notion documents) for relevant information",
            parameters: {
              type: "object",
              properties: {
                query: {
      ...
      if (responseMessage?.tool_calls && responseMessage.tool_calls.length > 0) {
        ...
        console.log(`[KnowledgeAgent] Step 4: Calling Groq for synthesis with tool results in history...`);
        ...
        if (searchResult.sources && searchResult.sources.length > 0) {
          const sourcesSection = "\n\n**Sources:**\n" + 
            searchResult.sources.slice(0, 3).map((s: any) => 
      ...
      const directContent = responseMessage?.content || "I can help you with that. What would you like to know?";

/**
 * ⚠️ DEPRECATED: Guide Routes
 *
 * This file contains the OLD guide progression system that has been replaced
 * by the WorkflowOptions metadata-based system.
 *
 * OLD FLOW (this file):
 * 1. Guide window shows step
 * 2. User clicks "Next" button in guide window
 * 3. POST /guides/progress called
 * 4. Backend progresses step, updates guide window
 * 5. Separate guide window displays new step
 *
 * NEW FLOW (WorkflowOptions):
 * 1. Conversation window shows WorkflowOptions component
 * 2. User clicks "Move on to next step" button
 * 3. Metadata sent to POST /conversations/:id/messages/stream
 * 4. Agent service receives metadata hint, selects guide_next_step tool
 * 5. Tool returns workflow message to conversation window (no separate window)
 *
 * This file is kept for reference only. The routes are commented out in routes.ts.
 * Can be deleted in future cleanup after confirming new system works properly.
 */

import { Router } from "express";
// DEPRECATED: Imports commented out to avoid unused import linter errors
...
// DEPRECATED: Commented out - see file header for explanation

...
/**
 * GET /api/integrations/gmail/folders
 * Get selected Gmail/Drive folders for Google Docs export
 */
router.get("/gmail/folders", requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = req.userId!;

    const [user] = await db
      .select({
        gmailSelectedFolders: schema.users.gmailSelectedFolders,
      })
      .from(schema.users)
      .where(eq(schema.users.id, userId))
      .limit(1);
    ...
    res.json({
      selectedFolderIds: user.gmailSelectedFolders || [],
    });
  } catch (error) {
    console.error("Error getting Gmail folders:", error);
    res.status(500).json({
      error: "Internal Server Error",
      message: "Failed to get Gmail folders",
    });
  }
});

/**
 * POST /api/integrations/gmail/folders
 * Save selected Gmail/Drive folders for Google Docs export
 */
router.post("/gmail/folders", requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = req.userId!;
    const { selectedFolderIds } = req.body;

    if (!Array.isArray(selectedFolderIds)) {
      res.status(400).json({
        error: "Bad Request",
        message: "selectedFolderIds must be an array",
      });
      return;
    }
    ...
    await db
      .update(schema.users)
      .set({
        gmailSelectedFolders: selectedFolderIds,
        updatedAt: new Date(),
      })
      .where(eq(schema.users.id, userId));

    console.log(`Gmail folders updated for user ${userId}: ${selectedFolderIds.length} folders`);
    ...
    console.error("Error saving Gmail folders:", error);
    res.status(500).json({
      error: "Internal Server Error",
      message: "Failed to save Gmail folders",
    });
  }
});

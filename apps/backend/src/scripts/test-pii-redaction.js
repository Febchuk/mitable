...
async function captureScreen() {
  console.log("📸 Capturing screenshot of primary monitor...");
  const sources = await desktopCapturer.getSources({
    types: ["screen"],
    thumbnailSize: {
      width: 2560, // Higher resolution for better OCR
      height: 1440,
    },
  });
  if (sources.length === 0) {
...
async function redactPII(screenshot) {
...
async function saveImages(original, redacted) {

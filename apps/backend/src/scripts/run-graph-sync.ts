...
async function run() {
  ...
  try {
    ...
    console.log(`  - Synced app behaviors: ${"syncedAppBehaviors" in result ? (result as any).syncedAppBehaviors : "n/a"}`);
    console.log(`  - Synced patterns: ${"syncedPatterns" in result ? (result as any).syncedPatterns : "n/a"}`);

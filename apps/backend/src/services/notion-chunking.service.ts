...
/**
 * NotionChunkingService - Structure-aware chunking for Notion documentation
 *
 * Strategy:
...
class NotionChunkingService {
  ...
  private chunkSection(section: NotionSection): NotionChunk[] {
    ...
    const flushTextChunk = () => {
      ...
      chunks.push({
        ...
        has_code: false,
        has_table: false,
        has_list: currentTextBlocks.some((b) => b.type.includes("list") || b.type === "to_do"),
    ...
    for (const block of section.blocks) {
      ...
      if (blockType === "code") {
        ...
        const text = sectionPrefix + block.text; // Already has [Code: language] prefix from service
   ...
   * Block text already has format: "[Code: language]\ncode..."
   */
  private extractCodeLanguage(block: NotionBlock): string {
    ...
    if (content.includes("curl ") || content.includes("npm ") || content.includes("git ")) {
      return "bash";
    }

  ...
  async redactScreenshot(request: PIIRedactionRequest): Promise<PIIRedactionResponse> {
    ...
    try {
      ...
      if (cached) {
        ...
        return {
          success: true,
          redactedScreenshot: cached,
          detectionTime: Date.now() - startTime,
          piiCount: 0, // Unknown for cached results
          cached: true,
        };
      }
      ...
      // Store in cache
      this.cache.set(cacheKey, redactedScreenshot);
      ...
      console.log(`[PIIRedactionService] Redaction complete: ${detectionTime}ms, cached: false`);
      ...
      return {
        success: false,
        redactedScreenshot: "", // Empty on failure
        detectionTime,
        piiCount: 0,
        cached: false,
